...
import { Query, Client, Account, Avatars, Databases, OAuthProvider } from 'react-native-appwrite';

export const config = {
  platform: 'com.sc.restate',
  endponint: process.env.EXPO_PUBLIC_APPWRITE_ENDPOINT,
...
client.setEndpoint(config.endponint!).setProject(config.projectId!).setPlatform(config.platform!);
...
export async function getProperties({ filter, query, limit }: { filter: string; query: string; limit?: number }) {
  try {
    ...
    if (query) {
      buildQuery.push(Query.or([Query.search('name', query), Query.search('type', query), Query.search('address', query)]));
    }
