...
const facilities = ['Laundry', 'Parking', 'Gym', 'Swimming pool', 'Wi-Fi', 'Pet Center'];
...
async function seed() {
  try {
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      const image =
        propertiesImages.length - 1 >= i ? propertiesImages[i] : propertiesImages[Math.floor(Math.random() * propertiesImages.length)];
